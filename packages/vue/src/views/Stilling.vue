  ...
  <main class="homepage container">
    <body class="stackedit mt-2">
      <div class="stackedit__html">
        ...
        <hr />
        <a :href="stilling.url"
          ><button type="button" class="btn btn-primary">Søk stilling</button></a
        >
        <br /><br />
        <hr />
        <h3 class="d-flex">
...
export default {
  setup() {
    ...
    function axiosMe() {
      axios
        .get(`http://localhost:3000/api/jobs?id=${route.currentRoute.value.params.id}`)
        .then(function (response) {
          console.log(response.data);
          stilling.value = response.data.data;
        })
